...
class CoinMarketCapClient:
    ...
    def __post_init__(self) -> None:
        ...
        if not key:
            raise CoinMarketCapError("COINMARKETCAP_API_KEY is not configured.")
    ...
    def _get(self, path: str, params: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not response.ok:
            ...
            raise CoinMarketCapError(f"CoinMarketCap HTTP {response.status_code}: {message}")
    ...
    def latest_quotes(
        self, symbols: list[str] | tuple[str, ...], convert: str = "USD"
    ) -> list[dict[str, Any]]:

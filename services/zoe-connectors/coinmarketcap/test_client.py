...
from .client import CoinMarketCapClient, CoinMarketCapError
...
class CoinMarketCapClientTests(unittest.TestCase):
    ...
    @patch("services.zoe-connectors.coinmarketcap.client.requests.get")
    def test_latest_quotes_uses_v3_endpoint(self, get):
        ...
        response.json.return_value = {
            "status": {"error_code": 0},
            "data": {
                "BTC": {"symbol": "BTC", "quote": {"USD": {"price": 1}}}
            },
        }
        get.return_value = response

        client = CoinMarketCapClient(api_key="test")
        result = client.latest_quotes(["BTC"])

        self.assertEqual(result[0]["symbol"], "BTC")
        self.assertEqual(get.call_args.args[0], "https://pro-api.coinmarketcap.com/v3/cryptocurrency/quotes/latest")
    ...
    def test_api_error_is_exposed_as_connector_error(self, get):
        ...
        client = CoinMarketCapClient(api_key="bad")
        with self.assertRaisesRegex(CoinMarketCapError, "API_KEY_INVALID"):
            client.latest_quotes(["BTC"])
